            ...
            <el-form :disabled="prometheusLoading">
              <el-row :gutter="10">
                ...
                <el-col :span="8">
                  ...
                  >
                    <el-select slot="append" v-model="integrationData.prometheus.interval[1]">
    ...
    const transformIntegrationData = (data) => {
      Object.keys(data).forEach((prop) => {
        switch (prop) {
          ...
          case 'interval':
            let matching =
              (typeof data[prop] === 'string' && data[prop].match(/(\d+)(\w)/)) ||
              (data[prop] instanceof Array && data[prop].unshift(data[prop].join('')) && data[prop])
